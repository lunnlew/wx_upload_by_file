...
UploadPic.prototype._handelSelectFile = function(ev) {
    ...
    //如果没有文件类型，则通过后缀名判断
    //微信仍在某些机型上不能获得后缀名
    //类似 image%3A123567 => image:123567
    
    if (!this.type) {
        var t = file.name.match(/\.([^\.]+)$/i);
        if(typeof t == "Array" && t.length==2){
            this.type = this.mime[t[1]];
        }else{
            //所以临时方案 固定为jpeg
...
UploadPic.prototype._getURI = function(file, callback) {
    ...
    function tmpLoad() {
        ...
        if (re.test(ret)) ret = ret.replace(re, 'data:' + _this.fileType + ';base64,');
...
UploadPic.prototype._drawImage = function(img, callback) {
    ...
    this.callback(this.canvas.toDataURL(this.type),this.input,this);
